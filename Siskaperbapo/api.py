...
class CommuditiyEastJava(object):
    ...
    def __get_options(self):
        ...
        city_selected = soup.find("select",{"id":"kabkota"})
        if city_selected:
            city_options = city_selected.find_all("option")
            options_list = [option.get_text(strip=True) for option in city_options if option.get_text(strip=True) != "Provinsi Jawa Timur"]
        self.driver.quit()
        return options_list
